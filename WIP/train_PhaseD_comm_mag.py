#MODEL_A_WEIGHTS_IN = "checkpoints\\combined_ab\\model_a_step5.weights.h5"
#MODEL_B_WEIGHTS_IN = "checkpoints\\combined_ab\\model_b_step5.weights.h5"
# Stage learning rates -> lr_a=9.999999747378752e-05 lr_b=0.00019999999494757503
# epoch 073  total=0.0567  shoot_acc=0.9995  comm_a_loss=0.0099  meas_in_a_loss=0.0000  comms_b_loss=0.0350  shoot_loss=0.0022  comm_mag_loss=0.0095  comm_mag_loss_raw=0.9499  mean_abs_comm_for_b=1.6968  comm_mag_beta_target=1.0000  w_comm_mag=0.0100  lr_a=0.0001  lr_b=0.0002  meas_in_a_per_0=0.0000  meas_in_a_per_1=0.0001  meas_in_a_per_2=0.0000  meas_in_a

# [weights] saved: model_a_best.weights.h5, model_b_best.weights.h5
# Early stopping criteria met at epoch 73.
# [log] flushed -> C:\Users\nly99857\OneDrive - Philips\SW Projects\QSeaBattle\WIP\logs\train_run_20260310_084132.pkl    
# [weights] saved: model_a_epoch_0073_final.weights.h5, model_b_epoch_0073_final.weights.h5
# [weights] saved: model_a_latest.weights.h5, model_b_latest.weights.h5
# {'log_path': 'C:\\Users\\nly99857\\OneDrive - Philips\\SW Projects\\QSeaBattle\\WIP\\logs\\train_run_20260310_084132.pkl', 'epochs_recorded': 74, 'interrupted': False}
#
# Stored as step 6
...
class TrainState:
    ...
    def __init__(self, depth: int, *, lr_a: float, lr_b: float, w_comm_mag: float):
...
def make_train_steps(depth: int):
    ...
    def _losses(
        ...
        mean_abs_err_comm_for_b = tf.reduce_mean(
                tf.abs(tf.abs(tf.cast(comm_logits_for_b, tf.float32)) - 1.0)
            )
        total = shoot_loss + comm_a_loss + meas_in_a_loss + comms_b_loss + comm_mag_loss
        ...
        return {
            ...
            "mean_abs_err_comm_for_b": mean_abs_err_comm_for_b,
        }
    ...
    def train_step_a(
        ...
        with tf.GradientTape() as tape:
            ...
            out = _losses(*vals, w_meas_in_a, w_comms_a, w_comms_b_123, w_shoot, w_comm_mag)
...
def train(config: dict[str, Any]) -> dict[str, Any]:
    ...
    state = TrainState(depth=DEPTH, lr_a=LR, lr_b=LR, w_comm_mag=W_COMM_MAG)
