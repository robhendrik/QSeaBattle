...
- Inputs are expected in the **scaled/logit** domains used in training:
  * field_batch: scaled values (typically {-0.5, +0.5})
  * sr_outcome_batch: logits (typically {−beta, +beta})
- Output is **logits** (no sigmoid).

Contract-aligned layer interface:
- call(field_batch, sr_outcome_batch, training=False) -> next_field_logits
- field_batch: tf.Tensor, shape (B, L)          # cropped; L is active width L_d
- sr_outcome_batch: tf.Tensor, shape (B, L/2)   # cropped; equals k_d
- next_field_logits: tf.Tensor, shape (B, L/2)  # cropped; equals L_{d+1}
...
class PyrCombineLayerA(tf.keras.layers.Layer):
    """Trainable layer mapping (field, sr_outcome) -> next_field logits."""
