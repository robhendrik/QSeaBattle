...
# Start with step 9 (models hardenen comms only, only adjusted B, scores 94% in gameplay)
# 
...
def make_train_steps(depth: int):
    ...
    def _core_forward(
        ...
        if mode == MODE_A_SHOOT:
            ...
            print("Warning: AB with comm hardening but no grad stop on comms from A to B")
            comm_base = comm_logits
            comm_hard = harden_ste(comm_base, beta=10.0)
            comm_logits_for_b = (1.0 - lambda_harden) * comm_base + lambda_harden * comm_hard
            meas_list_for_b = [harden_ste(t, beta=10.0) for t in meas_list]
            out_list_for_b = [tf.stop_gradient(t) for t in out_list]
            b_training_flag = True
...
def train(config: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        while epoch < EPOCHS:
            ...
            lambda_harden = tf.constant(float(1.0), dtype=tf.float32)
            _apply_stage(state, opt_a, opt_b, next_stage)
            print(f"\n[epoch {epoch}] in mode {int(state.TRAINING_MODE.numpy())} Starting epoch with lambda_harden={lambda_harden.numpy():.4f}")
            print(f"Learning rates: {opt_a.learning_rate.numpy():.2e} (A), {opt_b.learning_rate.numpy():.2e} (B)")  
