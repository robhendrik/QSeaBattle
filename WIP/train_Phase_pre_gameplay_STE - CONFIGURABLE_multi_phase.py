...
def _run_phase(
    *,
    ...
    LOG_FLUSH_EVERY_EPOCHS: int,
    global_epoch: int,
) -> tuple[int, bool]:
    stage_cfg      = phase["stage_cfg"]
    loss_cfg       = phase["loss_cfg"]
    interface_cfg  = phase["interface_cfg"]
    early_stop_cfg = phase["early_stop_cfg"]
    r_cfg          = phase["rollback_cfg"]
    max_epochs     = phase["max_epochs"]
    ENABLE_ROLLBACK          = r_cfg["ENABLE_ROLLBACK"]
    ROLLBACK_LOOKBACK        = r_cfg["LOOKBACK"]
    ROLLBACK_DROP_ABS        = r_cfg["DROP_ABS"]
    ROLLBACK_MIN_EPOCH       = r_cfg["MIN_EPOCH"]
    ROLLBACK_COOLDOWN_EPOCHS = r_cfg["COOLDOWN_EPOCHS"]
    best_tag = f"best_phase_{phase_idx}"

    phase_epoch        = 0
    early_stop_count   = 0
    interrupted        = False
    best_shoot_acc     = float("-inf")
    epoch_count_since_best = 0
    ...
    try:
        ...
        while phase_epoch < max_epochs:
            ...
            msg = f"\n[epoch {global_epoch}] Training mode: Model A {stage_cfg['update_a']}, Model B {stage_cfg['update_b']}"
            ...
            epoch_metrics: dict[str, Any] = {
                ...
                "epochs_since_best": float(epoch_count_since_best),
            }
            ...
            if epoch_metrics["shoot_acc"] > best_shoot_acc:
                ...
                save_model_weights(model_a, model_b, CHECKPOINT_DIR, tag=best_tag)
                best_stage_cfg = deepcopy(stage_cfg)
                best_interface_cfg = deepcopy(interface_cfg)
                best_loss_cfg = deepcopy(loss_cfg)
                epoch_count_since_best = 0
            else:
                epoch_count_since_best += 1
            ...
            if rollback_event["action"] == "rollback_best":
                best_a = _wfile(CHECKPOINT_DIR, "model_a", best_tag)
                best_b = _wfile(CHECKPOINT_DIR, "model_b", best_tag)
                rollback_loaded = load_model_weights(model_a, model_b, best_a, best_b)
                if rollback_loaded:
                    last_rollback_epoch = global_epoch
                    ...
                    opt_a = tf.keras.optimizers.Adam(stage_cfg["lr_a"])
                    opt_b = tf.keras.optimizers.Adam(stage_cfg["lr_b"])
            ...
            if LOG_FLUSH_EVERY_EPOCHS and ((global_epoch + 1) % LOG_FLUSH_EVERY_EPOCHS == 0):
                flush_run_log(run_log, run_log_path)
                print_verbosity(f"[log] flushed -> {run_log_path}", level=1, config=training_cfg)
            if SAVE_WEIGHTS_EVERY and ((global_epoch + 1) % SAVE_WEIGHTS_EVERY == 0):
                save_model_weights(model_a, model_b, CHECKPOINT_DIR, tag=f"epoch_{global_epoch + 1:04d}")
            ...
            if early_stop_count >= early_stop_cfg["consecutive"]:
                msg = f"Early stopping criteria met at epoch {global_epoch} "
    ...
    return global_epoch, interrupted
...
def train_multi_phase(
    ...
    effective_teacher_cfg = {
        **teacher_dataset_cfg,
        "DATASET_DIR": str(DATASET_DIR),
        "BATCH": BATCH,
        "SEED": SEED,
    }
    raw_ds = load_dataset(effective_teacher_cfg)
    tfds_train = build_train_pipeline(raw_ds, effective_teacher_cfg)
    ...
    phase0_updates = phases[0].get("model_updates", {})
    effective_model_cfg = {
        **model_cfg,
        "FIELD_SIZE": training_cfg["FIELD_SIZE"],
        "COMMS_SIZE": training_cfg["COMMS_SIZE"],
        "SEED": training_cfg["SEED"],
        "LAYOUT": LAYOUT,
        **{k: v for k, v in phase0_updates.items() if v is not None},
    }

    tf.random.set_seed(SEED)
    model_a = build_model_a(effective_model_cfg)
    model_b = build_model_b(effective_model_cfg)
    first_batch = next(iter(tfds_train))
    _force_build_models(model_a, model_b, first_batch)

    opt_a = tf.keras.optimizers.Adam(phases[0]["stage_cfg"]["lr_a"])
    opt_b = tf.keras.optimizers.Adam(phases[0]["stage_cfg"]["lr_b"])
    opt_a.build(model_a.trainable_variables)
    opt_b.build(model_b.trainable_variables)
    ...
    if training_cfg.get("LOG_NAME") is not None:
        run_log, run_log_path = init_run_log(
            LOG_DIR, run_load_event, log_name=training_cfg["LOG_NAME"])
    else:
        run_log, run_log_path = init_run_log(LOG_DIR, run_load_event)

    train_step = make_train_steps(depth=DEPTH)

    global_epoch = training_cfg.get("START_EPOCH", 0)
    interrupted = False
    phase_idx = 0
    ...
    for phase_idx, phase in enumerate(phases):
        ...
        updates = phase.get("model_updates", {})
        if updates.get("P_HIGH") is not None:
            model_a.set_p_high(updates["P_HIGH"])
            model_b.set_p_high(updates["P_HIGH"])
        if updates.get("ALPHA_FOR_PR_LAYERS") is not None:
            model_a.set_alpha(updates["ALPHA_FOR_PR_LAYERS"])
            model_b.set_alpha(updates["ALPHA_FOR_PR_LAYERS"])

        load_tag = phase.get("load_weights_from")
        if load_tag is not None:
            a_path = _wfile(CHECKPOINT_DIR, "model_a", load_tag)
            b_path = _wfile(CHECKPOINT_DIR, "model_b", load_tag)
            load_model_weights(model_a, model_b, a_path, b_path)

        if phase.get("reset_optimizer", True):
            opt_a = tf.keras.optimizers.Adam(phase["stage_cfg"]["lr_a"])
            opt_b = tf.keras.optimizers.Adam(phase["stage_cfg"]["lr_b"])
            opt_a.build(model_a.trainable_variables)
            opt_b.build(model_b.trainable_variables)

        global_epoch, interrupted = _run_phase(
            phase_idx=phase_idx,
            phase=phase,
            model_a=model_a,
            model_b=model_b,
    ...
    if SAVE_WEIGHTS_AT_END:
        end_tag = (
            f"epoch_{global_epoch:04d}_"
            f"{'interrupted' if interrupted else 'final'}"
        )
