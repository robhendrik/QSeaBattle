...
"""
Entire training pipeline (overview)
> Isolated A → learn protocol generation
> Isolated B → learn protocol decoding
> Phase A → stabilize joint AB system
> Phase B → improve joint performance
> Phase C → fine-tune and increase robustness
> Phase D → align logits with gameplay interface

Isolated A training
> purpose: teach Model A the correct communication protocol and measurement semantics using teacher supervision
> mode: A-only
> LR: moderate (stable convergence more important than speed)
> hardening: none
> noise: none
> anchors: teacher losses dominate
> target: near-perfect prediction of:
>> comms
>> meas_in
>> meas_out
> notes:
>> this stage defines the protocol language
>> logits can remain soft; no need to shape magnitude
>> stability and correctness matter more than confidence
>> this stage should converge to very high accuracy before moving on
> Typical result:
>> almost perfect teacher imitation
>> stable protocol outputs

Isolated B training
> purpose: teach Model B to interpret the communication protocol and produce correct shoot decisions
> mode: B-only
> LR: moderate
> hardening: none
> noise: none
...
> notes:
>> B learns to decode A’s protocol
>> this stage assumes protocol semantics are stable
>> logits again do not need magnitude shaping yet
> Typical result:
>> strong shoot accuracy when fed teacher protocol
>> stable decoding behavior

Phase A
> purpose: stabilize joint AB training from good isolated checkpoints
> mode: mostly AB
> LR: moderate; A slower than B
> hardening: none
> noise: none
> anchors: small but nonzero
> target: stable high performance without collaps

Phase B
> purpose: improve from stable Phase A
> mode: mostly AB
> LR: lower than Phase A
> hardening: none
> noise: none or tiny comm-only
> target: stable 99%+

Phase C
> purpose: fine-tune and make protocol robust
> mode: AB
> LR: lower again
> hardening: none
> noise: small comm-only allowed
> target: push above plateau, keep stability

Phase D
> purpose: prepare logits for real gameplay
> mode: AB
> LR: very low
> first add: sign-agnostic magnitude shaping on comm
> then add: interface hardening curriculum
> only later: extend to out, then maybe internal hardening
"""
