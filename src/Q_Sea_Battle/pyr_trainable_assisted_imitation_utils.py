...
"""
Q_Sea_Battle.pyr_trainable_assisted_imitation_utils
===================================================
...
----------
- :func:`pyramid_levels`
- :func:`generate_measurement_dataset_a_level`
- :func:`generate_combine_dataset_a_level`
- :func:`generate_measurement_dataset_b_level`
- :func:`generate_combine_dataset_b_level`
- :func:`to_tf_dataset`
- :func:`train_layer`
...
def teacher_combine_b(gun: np.ndarray, sr_outcome: np.ndarray, comm: np.ndarray):
    ...
    Inputs (per sample):
      ...
      - sr_outcome: shape (L//2,), shared randomness outcomes for this level.
...
def generate_measurement_dataset_a_level(L: int, num_samples: int, seed: int = 0) -> Dict[str, np.ndarray]:
...
def generate_combine_dataset_a_level(L: int, num_samples: int, seed: int = 0) -> Dict[str, np.ndarray]:
...
def generate_measurement_dataset_b_level(L: int, num_samples: int, seed: int = 0):
...
def generate_combine_dataset_b_level(L: int, num_samples: int, seed: int = 0):
    """Level-aware dataset for Pyramid Combine-B.

    Conventions (matching the game / players):
      - gun is one-hot over L positions
      - sr_outcome is binary over L//2 positions (shared randomness for this level)
    ...
    # Shared randomness outcomes for this level (binary, N x L//2)
