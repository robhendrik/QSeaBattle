"""Linear (learnable) measurement layer A.

Provides :class:`LinMeasurementLayerA`, a learnable mapping from a *field*
...
class LinMeasurementLayerA(tf.keras.layers.Layer):
    """Learnable mapping: field -> per-cell measurement probabilities in [0, 1]."""

    def __init__(
        self,
        n2: int,
        hidden_units: Sequence[int] = (64,),
        name: Optional[str] = "LinMeasurementLayerA",
        **kwargs,
    ) -> None:
    ...
    def call(self, fields, training: bool = False):
        """Forward pass.

        Args:
            fields: Tensor of shape (B, n2) or (n2,).
            training: Standard Keras training flag.

        Returns:
            Probabilities in [0, 1] with the same shape as `fields`.
        """
        x = tf.convert_to_tensor(fields)
