...
combine the *even* bits of the current field with the shared randomness outcome.
...
class PyrCombineLayerA(tf.keras.layers.Layer):
    ...
    Purpose:
        ...
        and a shared randomness outcome ``S_A^ℓ`` of length ``L/2``, compute:
    ...
    def call(self, field_batch: tf.Tensor, sr_outcome_batch: tf.Tensor) -> tf.Tensor:
        """Combine current field with shared randomness outcome.
