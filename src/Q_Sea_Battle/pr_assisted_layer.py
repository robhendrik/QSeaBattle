"""PRAssistedLayer (Keras layer).

This module implements the core PR-assisted primitive used by the
trainable assisted players.

It mirrors the classical :class:`~Q_Sea_Battle.pr_assisted.PRAssisted`
...
class _PRInputs:
    """Validated inputs for :class:`PRAssistedLayer`.
...
class PRAssistedLayer(tf.keras.layers.Layer):
    ...
    def _validate_inputs(self, inputs: Dict[str, tf.Tensor]) -> _PRInputs:
        ...
        if set(inputs.keys()) != required:
            ...
            raise ValueError(f"PRAssistedLayer.call expects keys {required}. Missing={missing}, extra={extra}")
