...
def test_expected_mode_matches_reference_binary() -> None:
    ...
    length = 8
    p_high = 0.8
    layer = PRAssistedLayer(length=length, p_high=p_high, mode="expected")
...
def test_sample_mode_reproducible_with_seed() -> None:
    ...
    p_high = 0.9
    seed = 123
    layer = PRAssistedLayer(length=length, p_high=p_high, mode="sample", resource_index=7, seed=seed)
...
def test_shapes_batched_and_unbatched() -> None:
    ...
    length = 8
    layer = PRAssistedLayer(length=length, p_high=0.75, mode="expected")
