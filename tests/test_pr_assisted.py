"""Tests for the PRAssisted helper class."""
...
def test_second_measurement_shape_and_values() -> None:
    ...
    _ = pr.measurement_a(meas_a)

    meas_b = np.array([0, 1, 0, 1], dtype=int)
    out_b = pr.measurement_b(meas_b)
...
def test_second_measurement_probabilistic_p_high_half() -> None:
    ...
    """
    length = 1
    pr = PRAssisted(length=length, p_high=0.5)
    ...
    for prev_bit, curr_bit in patterns:
        ...
        for _ in range(trials):
            pr.reset()
            prev_out = pr.measurement_a(np.array([prev_bit], dtype=int))
            out_b = pr.measurement_b(np.array([curr_bit], dtype=int))
